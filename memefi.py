...
# Handle all errors here SAFE_POST
...
def activate_energy_recharge_booster(index, headers):
    ...
    if response and 'data' in response and response['data'] and 'telegramGameActivateBooster' in response['data']:
        ...
        print("❌ Failed to activate Recharge Booster: Data incomplete or not available.")
...
def activate_booster(index, headers):
    ...
    if response and 'data' in response:
        ...
        if current_health == 0:
            print("\nBoss defeated, setting up next boss...")
...
def submit_taps(index, json_payload):
    ...
    response = safe_post(url, headers, json_payload)
    if response:
        return response  # Make sure to return the parsed data
    else:
        print(f"❌ Failed with status {response}, trying again...")
        return None  # Return None if an error occurs
...
def check_and_complete_tasks(index, headers):
    ...
    for task in tasks['data']['campaignTasks']:
        ...
        if task['name'] == "Follow telegram channel" and task['status'] == "Pending":
            ...
            continue  # Skip task if the task name is "Follow telegram channel" and the status is "Pending"
    ...
    # Recheck tasks after moving to verification
...
def main():
    ...
    print("\r Getting list of valid accounts...", end="", flush=True)
    ...
    while True:
        ...
        for index, result, first_name, last_name, league in accounts:
            ...
            if stat_result is not None:
                ...
                if darah_bos == 0:
                    print("\nBoss defeated, setting up next boss...", flush=True)
                    set_next_boss(index, headers)
                ...
                if energy_sekarang < 0.25 * user_data['maxEnergy']:
                    if auto_booster == 'y':
                        if user_data['freeBoosts']['currentRefillEnergyAmount'] > 0:
                            ...
                            print("\r🪫 Energy Depleted, no booster available. Switching to next account.\n", flush=True)
                            continue  # Switch to next account
                    else:
                        print("\r🪫 Energy Depleted, auto booster disabled. Switching to next account.\n", flush=True)
                        continue  # Switch to next account
...
def claim_combo(index, headers):
    ...
    if response and 'data' in response and 'telegramGameProcessTapsBatch' in response['data']:
        ...
        if game_data['tapsReward'] is None:
            ...
            print(f"✅ Combo claimed successfully: Taps reward {game_data['tapsReward']}")
    else:
        print("❌ Failed to claim combo: Data incomplete or not available.")
